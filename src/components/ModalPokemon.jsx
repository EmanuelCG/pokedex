...
const ModalPokemon = ({ showModal, onCloseModal, pokemon }) => {
    ...
    return (
        <section className={`fixed top-0 left-0 right-0 bg-green-400 h-screen transition-all duration-500 ${showModal ? "visible opacity-100" : "invisible opacity-0"}`}>
            ...
            <article className={`bg-white h-[85%] absolute w-full  rounded-tl-3xl rounded-tr-3xl text-center transition-all duration-500 ${showModal ? "visible opacity-100 bottom-0" : "-bottom-full"} px-4 grid gap-2 content-start overflow-auto`}>
                <header>
